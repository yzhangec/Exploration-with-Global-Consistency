...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  nh.param("exploration/alc_cost_scale", ep_->alc_cost_scale_, 1.0);
...
int FastExplorationManager::planExploreMotion(const Vector3d& pos, const Vector3d& vel,
  ...
  frontier_finder_->updateALCClusters(ed_->alc_points_, ed_->alc_yaws_, viewpoints_pos,
                                      viewpoints_yaw);
  computeActiveLoopClosureCosts(ed_->alc_points_, ed_->alc_yaws_, ed_->alc_costs, ed_->alc_paths);
  ...
  if (ed_->points_.size() > 1) {
    ...
    // The indices here is added with ALC points
    vector<int> indices;
    ...
    if (ep_->refine_local_) {
      ...
      for (int i = 0; i < knum; ++i) {
        Vector3d tmp;
        if (indices[i] < ed_->points_.size())
          tmp = ed_->points_[indices[i]];
        else
          tmp = ed_->alc_points_[indices[i] - ed_->points_.size()];
      ...
      if (ed_->refined_points_.size() == 0) {
        // ROS_WARN("ed_->refined_points_.size() == 0!");
        ed_->skip_alc_flag = true;
        return FAIL;
      } else {
        // ROS_WARN("ed_->refined_points_.size() == %d", ed_->refined_points_.size());
        ed_->skip_alc_flag = false;
      }
    ...
    if (ep_->refine_local_) {
      ...
      ed_->n_points_.clear();
      vector<vector<double>> n_yaws;
      frontier_finder_->getViewpointsInfo(pos, {0}, ep_->top_view_num_, ep_->max_decay_,
      ...
      for (int i = 0; i < ed_->n_points_[0].size(); ++i) {
        auto tmp_cost = ViewNode::computeCost(pos, ed_->n_points_[0][i], yaw[0], n_yaws[0][i], vel,
                                              yaw[1], tmp_path);
  ...
  if (len < 1.5) {
    ...
    if (!planner_manager_->kinodynamicReplan(pos, vel, acc, ed_->next_goal_, Vector3d(0, 0, 0),
                                             time_lb))
      return FAIL;
  }
    ...
    if ((path[i] - short_tour.back()).norm() > dist_thresh)
      ...
      while (ViewNode::caster_->nextId(idx) && ros::ok()) {
        if (edt_environment_->sdf_map_->getInflateOccupancy(idx) == 1 ||
            edt_environment_->sdf_map_->getOccupancy(idx) == SDFMap::OCCUPANCY::UNKNOWN) {
          short_tour.push_back(path[i]);
          break;
        }
      }
...
void FastExplorationManager::findGlobalTour(const Vector3d& cur_pos, const Vector3d& cur_vel,
  ...
  Eigen::MatrixXd cost_mat;      // from current state
  Eigen::MatrixXd cost_mat_alc;  // from nearest alc point
  Eigen::Vector3d nearest_alc_point;
  vector<int> indices_wo_alc, indices_alc;
  double nearest_alc_yaw;
  int nearest_alc_idx;

  frontier_finder_->getFullCostMatrix(cur_pos, cur_vel, cur_yaw, cost_mat);
  // frontier_finder_->getFullCostMatrixWithALCPoints(cur_pos, cur_vel, cur_yaw, ed_->alc_points_,
  //                                                  ed_->alc_yaws_, cost_mat);

  if (ed_->alc_points_.size() != 0) {
    frontier_finder_->getNearestALCPoint(cur_pos, cur_vel, cur_yaw, ed_->alc_points_,
                                         ed_->alc_yaws_, nearest_alc_point, nearest_alc_yaw,
                                         nearest_alc_idx);
    edt_environment_->sdf_map_->publishSingleViewpointsWithYaw(nearest_alc_point, nearest_alc_yaw);
    frontier_finder_->getFullCostMatrixFromNearestALCPoint(nearest_alc_point, nearest_alc_yaw,
                                                           cost_mat, cost_mat_alc);
  } else {
    edt_environment_->sdf_map_->publishSingleViewpointsWithYaw(Vector3d(0, 0, 0), -999.0);
  }
  ...
  while (getline(res_file, res)) {
    ...
    if (id == 1)  // Ignore the current state
      continue;
    if (id == -1) break;
    indices_wo_alc.push_back(id - 2);  // Idx of solver-2 == Idx of frontier
  }
  ...
  if (ed_->alc_points_.size() != 0) {
    ...
    for (int i = 0; i < dimension; ++i) {
      for (int j = 0; j < dimension; ++j) {
        int int_cost = cost_mat_alc(i, j) * scale;
        prob_file << int_cost << " ";
      }
      prob_file << "\n";
    }
    ...
    while (getline(res_file, res)) {
      ...
      if (id == 1)  // Ignore the current state
        continue;
      if (id == -1) break;
      indices_alc.push_back(id - 2);  // Idx of solver-2 == Idx of frontier
    }

    res_file.close();
  }

  double total_cost_wo_alc = 0.0, total_cost_alc = 0.0;

  // Calculate total cost w/o alc
  if (ed_->alc_points_.size() > 0) {
    int last_idx = 0;
    for (int i = 1; i < indices_wo_alc.size(); i++) {
      total_cost_wo_alc += cost_mat(last_idx, indices_wo_alc[i]);
    }

    // Calculate total cost w. alc
    last_idx = 0;
    for (int i = 1; i < indices_alc.size(); i++) {
      total_cost_alc += cost_mat_alc(last_idx, indices_alc[i]);
    }

    vector<Vector3d> path;
    total_cost_alc += ViewNode::computeCost(cur_pos, nearest_alc_point, cur_yaw[0], nearest_alc_yaw,
                                            cur_vel, cur_yaw[1], path);

    // ROS_WARN("total_cost_wo_alc: %f", total_cost_wo_alc);
    // ROS_WARN("total_cost_alc: %f", total_cost_alc);

    if (ros::Time::now() - ed_->expl_start_time_ < ros::Duration(20.0))
      ed_->skip_alc_flag = true;

    if (total_cost_alc < ep_->alc_cost_scale_ * total_cost_wo_alc && !ed_->skip_alc_flag) {
      indices.push_back(nearest_alc_idx + dimension - 1);
      indices.insert(indices.end(), indices_alc.begin(), indices_alc.end());
      ed_->alc_flag = true;
    } else {
      ed_->alc_flag = false;
      indices = indices_wo_alc;
    }
  } else {
    ed_->alc_flag = false;
    indices = indices_wo_alc;
...
void FastExplorationManager::computeActiveLoopClosureCosts(
    const vector<Eigen::Vector3d>& points, const vector<double>& yaws, Eigen::MatrixXd& costs,
    vector<vector<vector<Eigen::Vector3d>>>& paths) {
  paths.clear();
  Eigen::MatrixXd tmp_costs;
  tmp_costs.resize(points.size(), points.size());
  for (int i = 0; i < points.size(); i++) {
    vector<vector<Eigen::Vector3d>> paths_i;
    for (int j = i + 1; j < points.size(); j++) {
      vector<Vector3d> path_ij;
      double cost_ij = ViewNode::computeCost(points[i], points[j], yaws[i], yaws[j],
                                             Vector3d(0, 0, 0), 0, path_ij);
      // Insert item for both old and new clusters
      tmp_costs(i, j) = cost_ij;
      tmp_costs(j, i) = (cost_ij);
      paths_i.push_back(path_ij);
    }
    paths.push_back(paths_i);
  }
  costs = tmp_costs;
}
