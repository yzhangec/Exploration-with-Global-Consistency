...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  // frontier_timer_ = nh.createTimer(ros::Duration(0.5), &FastExplorationFSM::frontierCallback,
  // this);
  ...
  exploration_start_trigger_ = nh.advertiseService(
      "exploration_start", &FastExplorationFSM::explorationStartTriggerServiceCallback, this);
}
...
void FastExplorationFSM::FSMCallback(const ros::TimerEvent& e) {
  ...
  switch (state_) {
    ...
    case FINISH: {
      ...
      static bool is_save_tsdf = false;

      if (!is_save_tsdf) {
        // planner_manager_->edt_environment_->sdf_map_->saveTSDFMesh();
        // planner_manager_->edt_environment_->sdf_map_->eval2();
        is_save_tsdf = true;
      }
      preFrontierFind();
      break;
    ...
    case PLAN_TRAJ: {
      if (fd_->static_state_) {
        ...
        lock_guard<mutex> lock(fd_odom_mutex_);
    ...
    case EXEC_TRAJ: {
      ...
      if (t_cur > fp_->replan_thresh3_ && !classic_ && !expl_manager_->ed_->alc_flag) {
        transitState(PLAN_TRAJ, "FSM");
...
void FastExplorationFSM::visualize() {
  ...
  static int fr_num = 0;
  for (int i = ed_ptr->frontiers_.size(); i < fr_num; ++i) {
    visualization_->drawCubes({}, 0.1, Vector4d(0, 0, 0, 1), "frontier", i, 4);
    // visualization_->drawBox(Vector3d(0, 0, 0), Vector3d(0, 0, 0), Vector4d(1, 0, 0, 0.3),
    // "frontier_boxes", i, 4);
  }
  fr_num = ed_ptr->frontiers_.size();
  // for (int i = 0; i < ed_ptr->dead_frontiers_.size(); ++i)
  //   visualization_->drawCubes(ed_ptr->dead_frontiers_[i], 0.1, Vector4d(0, 0, 0, 0.5),
  //   "dead_frontier",
  //                             i, 4);
  ...
  visualization_->drawLines(ed_ptr->global_tour_, 0.07, Vector4d(0, 0.5, 0, 1), "global_tour", 0,
                            6);
  // visualization_->drawLines(ed_ptr->points_, ed_ptr->views_, 0.05, Vector4d(0, 1, 0.5, 1),
  // "view",
  //                           0, 6);
  ...
  // visualization_->drawSpheres(ed_ptr->refined_points_, 0.2, Vector4d(0, 0, 1, 1), "refined_pts",
  // 0,
  //                             6);
  // visualization_->drawLines(ed_ptr->refined_points_, ed_ptr->refined_views_, 0.05,
  //                           Vector4d(0.5, 0, 1, 1), "refined_view", 0, 6);
  // visualization_->drawLines(ed_ptr->refined_tour_, 0.07, Vector4d(0, 0, 1, 1), "refined_tour", 0,
  //                           6);
  ...
  // Draw trajectory
  // visualization_->drawSpheres({ed_ptr->next_goal_}, 0.3, Vector4d(0, 1, 1, 1), "next_goal", 0,
  // 6);
...
void FastExplorationFSM::preFrontierFind() {
  static ros::Time t;
  if (ros::Time::now() - t < ros::Duration(0.5)) return;
  t = ros::Time::now();
  auto ft = expl_manager_->frontier_finder_;
  auto ed = expl_manager_->ed_;
...
void FastExplorationFSM::triggerCallback(const nav_msgs::PathConstPtr& msg) {
  ...
  fd_->trigger_ = true;
  expl_manager_->ed_->expl_start_time_ = ros::Time::now();
  ROS_INFO("[FastExplorationFSM] The exploration tast is triggered at %f", expl_manager_->ed_->expl_start_time_);
  transitState(PLAN_TRAJ, "triggerCallback");
}

bool FastExplorationFSM::explorationStartTriggerServiceCallback(
    std_srvs::Empty::Request& request, std_srvs::Empty::Response& response) {
  if (state_ != WAIT_TRIGGER) return false;
  fd_->trigger_ = true;
  expl_manager_->ed_->expl_start_time_ = ros::Time::now();
  ROS_INFO("[FastExplorationFSM] The exploration tast is triggered at %f", expl_manager_->ed_->expl_start_time_);
  transitState(PLAN_TRAJ, "explorationStartTriggerServiceCallback");
  return true;
}
