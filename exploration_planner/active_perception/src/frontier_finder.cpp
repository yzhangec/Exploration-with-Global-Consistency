...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr &edt, ros::NodeHandle &nh) {
  ...
  nh.param("frontier/show_cost_mat", show_cost_mat_, true);
...
void FrontierFinder::searchFrontiers() {
  ...
  edt_env_->sdf_map_->publishBoundingBox(update_min, update_max);
...
void FrontierFinder::updateFrontierCostMatrix() {
  ...
  auto updateCost = [this](const list<Frontier>::iterator &it1,
                           const list<Frontier>::iterator &it2) {
    if (this->show_cost_mat_) {
  ...
  for (auto it1 = first_new_ftr_; it1 != frontiers_.end(); ++it1)
    for (auto it2 = it1; it2 != frontiers_.end(); ++it2) {
      if (it1 == it2) {
        if (show_cost_mat_) std::cout << "(" << it1->id_ << "," << it2->id_ << "), ";
...
void FrontierFinder::getViewpointsInfo(const Vector3d &cur_pos, const vector<int> &ids,
  ...
  for (auto id : ids) {
    if (id < frontiers_.size()) {
      ...
      vector<Eigen::Vector3d> pts;
      vector<double> ys;
      for (Viewpoint view : alc_clusters_[id - frontiers_.size()].viewpoints_) {
...
void FrontierFinder::getFrontiers(vector<vector<Eigen::Vector3d>> &clusters) {
  // sometimes have destructor error
  clusters.clear();
...
void FrontierFinder::getPathForTour(const Vector3d &pos, const vector<int> &frontier_ids,
  ...
  vector<vector<ALCCluster>::iterator> frontier_and_alc_clusters_indexer;
  for (auto it = frontier_and_alc_clusters_.begin(); it != frontier_and_alc_clusters_.end(); ++it)
    frontier_and_alc_clusters_indexer.push_back(it);

  vector<vector<ALCCluster>::iterator> alc_clusters_indexer;
  for (auto it = alc_clusters_.begin(); it != alc_clusters_.end(); ++it)
    alc_clusters_indexer.push_back(it);

  // ROS_WARN("frontiers_ size: %d", frontiers_.size());
  // ROS_WARN("frontier_and_alc_clusters_ size: %d", frontier_and_alc_clusters_.size());
  // ROS_WARN("alc_clusters_ size: %d", alc_clusters_.size());

  // Compute the path from current pos to the first frontier
  vector<Vector3d> segment;

  if (frontier_ids[0] < frontier_indexer.size()) {
    ViewNode::searchPath(pos, frontier_indexer[frontier_ids[0]]->viewpoints_.front().pos_, segment);
    path.insert(path.end(), segment.begin(), segment.end());
  } else {
    ViewNode::searchPath(pos, alc_clusters_indexer[frontier_ids[0] - frontier_indexer.size()]->center_position_, segment);
    path.insert(path.end(), segment.begin(), segment.end());
  }

  for (int i = 0; i < frontier_ids.size() - 1; ++i) {
    if (frontier_ids[i] < frontier_indexer.size() &&
        frontier_ids[i + 1] < frontier_indexer.size()) {
      // ROS_WARN_STREAM("get path from frontier to frontier" << frontier_ids[i] << " to "
      //                                                      << frontier_ids[i + 1]);
    ...
    } else if (frontier_ids[i] >= frontier_indexer.size() &&
               frontier_ids[i + 1] < frontier_indexer.size()) {
      // ROS_WARN_STREAM("get path from alc points to frontier" << frontier_ids[i] << " to "
      //                                                        << frontier_ids[i + 1]);
      vector<Vector3d> tmp_path;
      ViewNode::searchPath(
          alc_clusters_indexer[frontier_ids[i] - frontier_indexer.size()]->center_position_,
          frontier_indexer[frontier_ids[i + 1]]->viewpoints_.front().pos_, tmp_path);
      path.insert(path.end(), tmp_path.begin(), tmp_path.end());
    } else if (frontier_ids[i] < frontier_indexer.size() &&
               frontier_ids[i + 1] >= frontier_indexer.size()) {
      // ROS_WARN_STREAM("get path from frontier to alc points" << frontier_ids[i] << " to "
      //                                                        << frontier_ids[i + 1]);
      vector<Vector3d> tmp_path;
      ViewNode::searchPath(
          frontier_indexer[frontier_ids[i]]->viewpoints_.front().pos_,
          alc_clusters_indexer[frontier_ids[i + 1] - frontier_indexer.size()]->center_position_,
          tmp_path);
      path.insert(path.end(), tmp_path.begin(), tmp_path.end());
    }
  }
  ...
  //   // ROS_WARN_STREAM("getPathForTour: " << frontier_ids[i]
  //   //                                    << ", frontier_indexer size: " <<
  //   //                                    frontier_indexer.size());
  //   if (frontier_ids[i] < frontier_indexer.size() &&
  //       frontier_ids[i + 1] < frontier_indexer.size()) {
  //     ROS_WARN_STREAM("get path from frontier to frontier" << frontier_ids[i] << " to "
  ...
  //   } else if (frontier_ids[i] >= frontier_indexer.size() &&
  //              frontier_ids[i + 1] < frontier_indexer.size()) {
  //     // get path from alc points to frontier
  //     ROS_WARN_STREAM("get path from alc points to frontier" << frontier_ids[i] << " to "
  //                                                            << frontier_ids[i + 1]);
  //     auto path_iter = frontier_and_alc_clusters_indexer[frontier_ids[i + 1]]->paths_.begin();
  //     int next_idx = frontier_ids[i] - frontier_indexer.size();
  //     for (int j = 0; j < next_idx; ++j) ++path_iter;
  //     reverse(path_iter->begin(), path_iter->end());
  //     path.insert(path.end(), path_iter->begin(), path_iter->end());
  //   } else if (frontier_ids[i] < frontier_indexer.size() &&
  //              frontier_ids[i + 1] >= frontier_indexer.size()) {
  //     // get path from frontier to alc points
  //     ROS_WARN_STREAM("get path from frontier to alc points" << frontier_ids[i] << " to "
  //                                                            << frontier_ids[i + 1]);
  //     auto path_iter = frontier_and_alc_clusters_indexer[frontier_ids[i]]->paths_.begin();
  //     int next_idx = frontier_ids[i + 1] - frontier_indexer.size();
  //     for (int j = 0; j < next_idx; ++j) ++path_iter;
  //     path.insert(path.end(), path_iter->begin(), path_iter->end());
  //   } else if (frontier_ids[i] >= frontier_indexer.size() &&
  //              frontier_ids[i + 1] >= frontier_indexer.size()) {
  //     // get path from alc points to alc points
  //     ROS_WARN_STREAM("get path from alc points to alc points: " << frontier_ids[i] << " to "
  //                                                                << frontier_ids[i + 1]);
  //     if (frontier_ids[i] < frontier_ids[i + 1]) {
  //       auto path_iter =
  //           alc_clusters_indexer[frontier_ids[i] - frontier_indexer.size()]->paths_.begin();
  //       int next_idx = frontier_ids[i + 1] - frontier_indexer.size();
  //       next_idx -= frontier_ids[i] - frontier_indexer.size();
  //       for (int j = 1; j < next_idx; ++j) ++path_iter;
  //       path.insert(path.end(), path_iter->begin(), path_iter->end());
  //     } else {
  //       auto path_iter =
  //           alc_clusters_indexer[frontier_ids[i + 1] - frontier_indexer.size()]->paths_.begin();
  //       int next_idx = frontier_ids[i] - frontier_indexer.size();
  //       next_idx -= frontier_ids[i + 1] - frontier_indexer.size();
  //       for (int j = 1; j < next_idx; ++j) ++path_iter;
  //       reverse(path_iter->begin(), path_iter->end());
  //       path.insert(path.end(), path_iter->begin(), path_iter->end());
  //     }
...
void FrontierFinder::getFullCostMatrix(const Vector3d &cur_pos, const Vector3d &cur_vel,
  ...
  if (false) {
    ...
    // Costs from depot to sites, the same large value
    for (j = 1; j < dim - 1; ++j) {
  ...
  // cout << "Cost mat (original): " << endl << mat << endl;
}

void FrontierFinder::getFullCostMatrixWithALCPoints(const Vector3d &cur_pos,
                                                    const Vector3d &cur_vel, const Vector3d cur_yaw,
                                                    const vector<Eigen::Vector3d> &alc_points,
                                                    const vector<double> &alc_yaw,
                                                    Eigen::MatrixXd &mat) {
  // Use Asymmetric TSP
  int dimen = frontiers_.size() + alc_points.size();
  mat.resize(dimen + 1, dimen + 1);
  // Fill block for clusters
  int i = 1, j = 1;
  for (auto ftr : frontiers_) {
  ...
  // cost among frontiers and alc points
  i = 1, j = 0;
  frontier_and_alc_clusters_.clear();
  for (auto ftr : frontiers_) {
    ALCCluster cluster;
    for (j = 0; j < alc_points.size(); j++) {
      vector<Vector3d> path_ij;
      double cost_ij =
          ViewNode::computeCost(alc_points[j], ftr.viewpoints_.front().pos_, alc_yaw[j],
                                ftr.viewpoints_.front().yaw_, Vector3d(0, 0, 0), 0, path_ij);
      mat(i, j + frontiers_.size() + 1) = cost_ij;
      mat(j + frontiers_.size() + 1, i) = cost_ij;
      cluster.paths_.push_back(path_ij);
    }
    ++i;
    frontier_and_alc_clusters_.push_back(cluster);
    // ROS_WARN_STREAM("frontier_and_alc_clusters_ size: " << frontier_and_alc_clusters_.size());
  }

  // cost among alc points
  for (int i = 0; i < alc_points.size(); i++) {
    for (int j = i; j < alc_points.size(); j++) {
      if (i == j) {
        mat(i + frontiers_.size() + 1, j + frontiers_.size() + 1) = 0.0;
        continue;
      }

      vector<Vector3d> path_ij;
      double cost_ij = ViewNode::computeCost(alc_points[i], alc_points[j], alc_yaw[i], alc_yaw[j],
                                             Vector3d(0, 0, 0), 0, path_ij);
      mat(i + frontiers_.size() + 1, j + frontiers_.size() + 1) = cost_ij;
      mat(j + frontiers_.size() + 1, i + frontiers_.size() + 1) = cost_ij;

      // paths from i to i+1, i+2, ...
      alc_clusters_[i].paths_.push_back(path_ij);
      // ROS_WARN("alc_clusters_[%d].paths_.push_back(path_ij), j: %d, path_size: %d", i, j,
      //          path_ij.size());
    }
    // ROS_WARN_STREAM("alc_clusters_ size: " << alc_clusters_.size());
  }
  ...
  for (int i = 0; i < alc_points.size(); i++) {
    vector<Vector3d> path;
    mat(0, j++) = ViewNode::computeCost(cur_pos, alc_points[i], cur_yaw[0], alc_yaw[i], cur_vel,
                                        cur_yaw[1], path);
  }

  // std::cout << "Cost matrix: " << endl << mat << endl;
}

void FrontierFinder::getNearestALCPoint(const Vector3d &cur_pos, const Vector3d &cur_vel,
                                        const Vector3d cur_yaw,
                                        const vector<Eigen::Vector3d> &alc_points,
                                        const vector<double> &alc_yaws, Eigen::Vector3d &alc_point,
                                        double &alc_yaw, int &idx) {
  if (alc_points.size() == 0) return;

  vector<double> cost_vector;
  for (int i = 0; i < alc_points.size(); i++) {
    vector<Vector3d> path;
    double cost = ViewNode::computeCost(cur_pos, alc_points[i], cur_yaw[0], alc_yaws[i], cur_vel,
                                        cur_yaw[1], path);
    cost_vector.push_back(cost);
  }

  idx = std::min_element(cost_vector.begin(), cost_vector.end()) - cost_vector.begin();
  alc_point = alc_points[idx];
  alc_yaw = alc_yaws[idx];
}

void FrontierFinder::getFullCostMatrixFromNearestALCPoint(const Eigen::Vector3d &alc_point,
                                                          const double &alc_yaw,
                                                          const Eigen::MatrixXd &mat_original,
                                                          Eigen::MatrixXd &mat) {
  int dimen = frontiers_.size();
  mat.resize(dimen + 1, dimen + 1);

  mat.block(1, 1, dimen, dimen) = mat_original.block(1, 1, dimen, dimen);
  ...
  for (auto ftr : frontiers_) {
    ...
    vector<Vector3d> path;
    mat(0, j++) =
        ViewNode::computeCost(alc_point, vj.pos_, alc_yaw, vj.yaw_, Vector3d(0, 0, 0), 0, path);
  }

  // cout << "Cost mat (ALC): " << endl << mat << endl;
}
  ...
  for (double phi_h = -M_PI_2; phi_h <= M_PI_2 + 1e-3; phi_h += M_PI / 18) {
    ...
    for (double phi_v = -vertical_angle_; phi_v <= vertical_angle_; phi_v += vertical_angle_ / 3) {
      ...
      while (raycaster_->nextId(idx)) {
        ...
        if (edt_env_->sdf_map_->getOccupancy(idx) == SDFMap::OCCUPANCY::UNKNOWN) ++gain;
      }
    }
  ...
  for (int x = -vox_num; x <= vox_num; ++x)
    for (int y = -vox_num; y <= vox_num; ++y)
      for (int z = -1; z <= 1; ++z) {
        ...
        if (edt_env_->sdf_map_->getOccupancy(vox) == SDFMap::OCCUPANCY::UNKNOWN) return true;
      }
  return false;
}
  ...
  for (auto cell : cluster) {
    ...
    while (raycaster_->nextId(idx)) {
      if (edt_env_->sdf_map_->getInflateOccupancy(idx) == 1 ||
          edt_env_->sdf_map_->getOccupancy(idx) == SDFMap::OCCUPANCY::UNKNOWN) {
        visib = false;
        break;
      }
    }
...
inline bool FrontierFinder::isNeighborUnknown(const Eigen::Vector3i &voxel) {
  ...
  for (auto nbr : nbrs) {
    if (edt_env_->sdf_map_->getOccupancy(nbr) == SDFMap::OCCUPANCY::UNKNOWN) return true;
  }
  return false;
}
...
inline bool FrontierFinder::knownfree(const Eigen::Vector3i &idx) {
  return edt_env_->sdf_map_->getOccupancy(idx) == SDFMap::OCCUPANCY::FREE;
}
...
void FrontierFinder::updateALCClusters(const vector<Eigen::Vector3d> &points,
                                       const vector<double> &yaws,
                                       const vector<vector<Eigen::Vector3d>> &viewpoints_pos,
                                       const vector<vector<double>> &viewpoints_yaw) {
  alc_clusters_.clear();

  if (points.size() != yaws.size()) {
    ROS_ERROR("points size and yaw size different");
  }

  // ROS_ERROR_STREAM("points size: " << points.size());
  // ROS_ERROR_STREAM("yaws size: " << yaws.size());
  // ROS_ERROR_STREAM("viewpoints_pos size: " << yaws.size());

  for (int i = 0; i < points.size(); i++) {
    ALCCluster cluster;
    cluster.center_position_ = points[i];
    cluster.center_yaw_ = yaws[i];

    for (int j = 0; j < viewpoints_pos[i].size(); j++) {
      Viewpoint vp;
      vp.pos_ = viewpoints_pos[i][j];
      vp.yaw_ = viewpoints_yaw[i][j];
      cluster.viewpoints_.push_back(vp);
    }

    alc_clusters_.push_back(cluster);
  }
}
