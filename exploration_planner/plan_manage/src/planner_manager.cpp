...
void FastPlannerManager::planExploreTraj(const vector<Eigen::Vector3d>& tour,
  ...
  std::cout << "pos size: " << pos.size() << ", cur_vel: " << cur_vel.transpose()
            << ", cur_acc: " << cur_acc.transpose() << "time size: " << times.size() << endl;
...
//   std::cout << "dt_yaw: " << dt_yaw << ", start yaw: " << start_yaw3d.transpose() << ", end: " <<
//   end_yaw << std::endl;
...
//   // Initial state
//   Eigen::Matrix3d states2pts;
//   states2pts << 1.0, -dt_yaw, (1 / 3.0) * dt_yaw * dt_yaw, 1.0, 0.0, -(1 / 6.0) * dt_yaw *
//   dt_yaw, 1.0, dt_yaw,
//       (1 / 3.0) * dt_yaw * dt_yaw;
