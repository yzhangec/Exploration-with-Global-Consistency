...
void pgTVioCallback(geometry_msgs::Pose msg) {
  Eigen::Quaterniond q = Eigen::Quaterniond(msg.orientation.w, msg.orientation.x, msg.orientation.y,
                                            msg.orientation.z);
...
void cmdCallback(const ros::TimerEvent& e) {
  // (X) No publishing before receive traj data
  // changed to hover at 0, 0, 1 temporarily
  if (!receive_traj_) {
